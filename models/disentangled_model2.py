...
"""
Jointly estimate transmission t(x) and airlight A
"""
...
class DisentangledModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netAir = networks.define_G(input_nc=opt.input_nc, output_nc=3, ngf=6,    # null input
                                      which_model_netG='air', gpu_ids=self.gpu_ids, n_layers=opt.n_layers)
        ...
        if not self.isTrain or opt.continue_train:
            ...
            self.load_network(self.netAir, 'Air', opt.which_epoch)
            if self.isTrain:
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG.parameters(), self.netDepth.parameters(), self.netAir.parameters()),
        ...
        networks.print_network(self.netAir)
        if self.isTrain:
    ...
    def forward(self):
        ...
        self.air = self.netAir.forward(self.real_A * (1-self.depth))

        # reconstruct A based on optical model
        self.fake_A = util.synthesize_matting(self.fake_B, self.depth, self.air.expand_as(self.fake_B))
    ...
    def get_current_visuals(self):
        ...
        return OrderedDict([('Hazy', real_A), ('Haze-free', fake_B), ('Estimate_depth', fake_depth), 
            ('recover', fake_A), ('real_depth', real_depth), ('real_B', real_B)])

    def save(self, label):
        self.save_network(self.netG, 'G', label, self.gpu_ids)
        self.save_network(self.netDepth, 'Depth', label, self.gpu_ids)
        self.save_network(self.netAir, 'Air', label, self.gpu_ids)
